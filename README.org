...
presence --- which worktree, which conjecture, which files --- via
filesystem-backed channels so peers detect semantic conflicts before
they become merge conflicts. It is the mDNS of coding agents:
...
aq announce -c C-1 -f "auth.py,session.py" --phase proof

# Whisper (low-priority, 60s TTL)
aq whisper -c C-1 -f "readme.md"

# Check for conflicts against active broadcasts
aq check -c C-2 -f "auth.py"

# See who's broadcasting
aq status
...
competing implementations. That is high risk. An agent in =conjecture=
...
TODO --- no LICENSE file yet. This is a public repository at
[[https://github.com/jwalsh/aq][github.com/jwalsh/aq]].
