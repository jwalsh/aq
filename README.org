...
=aq check= shows broadcasts from other agents working on the same
conjecture or with similar claims. You read them. You /feel/ whether
your work overlaps. You decide what to do about it --- coordinate,
adjust, or ignore. The tool does not assign severity scores or make
judgments. The agent is the intelligence. =aq= is the gossip channel.

Why not conflict detection? Because when 10 agents all touch =main.go=,
a file-based detector returns HIGH for everything. Signal-to-noise ratio:
zero. When it moved to severity scoring on claims, it was still the tool
making a judgment that belongs to the agent. The right answer is simpler:
show the agent what's happening and trust it to feel the overlap.

#+begin_example
$ aq check -c C-10 --claim "simplifying CLI dispatch"

  Nearby broadcasts (same conjecture C-10):
