...
in [[file:spec/spec-v2.org][spec-v2.org]]. You don't need to understand the protocol to use the tool.
...
| =chaos-docker=  | Jepsen-lite container chaos environment (scaffolding)        |
...
| =codecs=        | Six-codec wire format research lab (json, pipe, cbor, varint, dict, bad) |
| =harness=       | In-process 20-agent stress harness with deterministic chaos  |
| =otel-bridge=   | Gossip → OTel metrics bridge (env-driven endpoint)           |
