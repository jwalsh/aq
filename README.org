...
[[https://github.com/aygp-dr/tramp-workshop][Emacs TRAMP]] is a Tier 0.5 transport that deserves special mention.
TRAMP makes remote filesystems transparent to Emacs --- multi-hop SSH,
sudo escalation, and even FreeBSD Bastille jail access all collapse into
a single path. Since aq's filesystem transport is just =read()= and
=write()= on =AQ_HOME=, TRAMP-accessed paths work without modification:

#+begin_src emacs-lisp
;; Agent in a Bastille jail broadcasts via TRAMP
(setq aq-home "/ssh:operator@sandbox|bastille:agent-alpha:/root/.aq/")
#+end_src

This matters for agentic isolation: each agent runs in its own jail with
its own =~/.aq/=, but the operator sees all broadcasts from Emacs via
TRAMP hops. The gossip layer follows the same transparent-FS principle
as NFS/SMB --- it doesn't know or care that the files are remote.

See [[https://github.com/aygp-dr/tramp-workshop][aygp-dr/tramp-workshop]] for the full multi-hop topology.
